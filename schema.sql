...
BEGIN 
    ...
    FROM (SELECT OrderID,productID,INVENTRY.ProductPrice*ORDER_PRODUCTS.ProductQuantity AS total FROM INVENTORY INNER JOIN ORDER_PRODUCTS ON INVENTORY.ProductID=INVENTORY.ProductIDINVENTORY.ProductID)a
    GROUP BY a.OrderID;
END $orderFigure$
LANGUAGE plpgsql;
